...
def prepare_2d_slices(split_name):
    # 将3D卷切分为2D切片
    src_img_dir = f"dataset/{split_name}/images"
    src_lab_dir = f"dataset/{split_name}/labels"

    # 创建存放2D切片的目录
    save_dir = Path(f"dataset/{split_name}_2d")
    ...
    for f in tqdm(files, file=sys.stdout):
        ...
        for z in range(img_array.shape[0]):
            ...
            if np.sum(slice_lab) == 0:
                if random.random() > 0.2:  # 生成一个 0~1 的随机数，大于 0.2 就跳过
                    continue  # 只保留 20%
...
if __name__ == "__main__":
    print("[INFO] 3D to 2D Data Preprocessing Started")
    set_seed(42)
    prepare_2d_slices("train")
    prepare_2d_slices("val")
    print("")
    print("[INFO] Preprocessing complete! 2D slices saved to 'train_2d' and 'val_2d' folders.")
